...
class BlockData(@Expose val id: String) {
    ...
    fun openEdit(player: Player) {
        MenuBuilder.builder()
                ...
                .rows(3)
                .build { inv ->
                    inv.setItem(12, ItemBuilder(Materials.STRUCTURE_VOID.parseMaterial()).name("§f阶段 (${progress.size})").lore(progress.mapIndexed { index, progress ->
                    ...
                    inv.setItem(14, ItemBuilder(Materials.BOOKSHELF.parseMaterial()).name("§f生长").lore("§7时间: ${growTime}秒 §8(左键编辑)", "§7几率: ${growChange * 100}% §8(右键编辑)").build())
                }.event {
                    it.isCancelled = true
                    when (it.rawSlot) {
                        12 -> {
        ...
        when {
            ...
            openStructure == null -> {
                ...
                MenuBuilder.builder()
                        ...
                        .rows(3)
                        .build { inv ->
                            inv.setItem(11, ItemBuilder(Materials.GLASS.parseMaterial()).name("§f替换").lore("§7${Items.getName(ItemStack(openStructure.replace))}").build())
                            inv.setItem(13, ItemBuilder(Materials.IRON_PICKAXE.parseMaterial()).name("§f工具").lore("§7${openStructure.tool ?: "无"}").build())
                            inv.setItem(15, ItemBuilder(Materials.CHEST_MINECART.parseMaterial()).name("§f掉落").lore(openStructure.drop.map { "§7${it.item} * ${it.amount} (${it.chance * 100}%)" }).build())
                        }.event {
                            it.isCancelled = true
                            when (it.rawSlot) {
                                11 -> {
